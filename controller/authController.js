...
const signUp = async (req,res) => {
    try {
        const data = req.body // recieve the data from front end
        ...
        // validation with model and save data in db collection
...
const login = async (req,res) => {
    try {
            ...
            if (data.email) {
                ...
                if (!extMobile) {
                    return res.status(StatusCodes.NOT_FOUND).json({ msg: `${data.mobile} not found`})
...
const generatePassLink = async (req,res) => {
    try {
        ...
        if(!extUser)
            ...
            res.status(StatusCodes.ACCEPTED).json({ msg: `token sent successfully..check your email inbox/spam folder`, savedToken })
    ...
    // validate the password link and update password logic
